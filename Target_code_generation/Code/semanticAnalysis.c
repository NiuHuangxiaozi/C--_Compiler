...
void _Program(TreeNode *root) {
	if (root == NULL)return;
	_ExtDefList((TreeNode *) root->child[0]);
}
...
//empty//
	if (root->is_terminal == 0 && root->child_num == 0)
		...
		_ExtDef((TreeNode *) root->child[0]);
		_ExtDefList((TreeNode *) root->child[1]);
	}
}
...
void _ExtDef(TreeNode *root) {
	...
	//Get Specifier
	TreeNode *node_1 = (TreeNode *) root->child[1];
	Type *_type = (Type *) malloc(sizeof(Type));  //acquire type
	initial_type(_type);

	if (strcmp(node_1->name, "ExtDecList") == 0) {
		_Specifier((TreeNode *) root->child[0], _type);
		_ExtDecList((TreeNode *) root->child[1], _type);
		_SEMI((TreeNode *) root->child[2]);
	} else if (strcmp(node_1->name, "SEMI") == 0) {
		_Specifier((TreeNode *) root->child[0], _type);
		_SEMI((TreeNode *) root->child[1]);
	} else if (strcmp(node_1->name, "FunDec") == 0) {
		_Specifier((TreeNode *) root->child[0], _type);
		_FunDec((TreeNode *) root->child[1], _type);
		_CompSt((TreeNode *) root->child[2]);
...
void _ExtDecList(TreeNode *root, Type *_type) {
	...
	{
		_VarDec((TreeNode *) root->child[0], _type);
	} else//3
	{
		_VarDec((TreeNode *) root->child[0], _type);
		_ExtDecList((TreeNode *) root->child[2], _type);
	}
}
...
void _Specifier(TreeNode *root, Type *_type) {
	...
	TreeNode *node_0 = (TreeNode *) root->child[0];
	if (strcmp(node_0->name, "TYPE") == 0) {
		_TYPE((TreeNode *) root->child[0], _type);
	} else {
		_StructSpecifier((TreeNode *) root->child[0], _type);
	}
}
...
void _StructSpecifier(TreeNode *root, Type *_type) {
	...
	if (root->child_num == 5) {
		...
		_type->type = Struct;
		//get name of struct
		_OptTag((TreeNode *) root->child[1], _type);// acquire name
		...
		//get into field
		set_in_struct();
		_DefList((TreeNode *) root->child[3]);
		set_out_struct();
	...
	} else //2
	{
		...
		_Tag((TreeNode *) root->child[1], _type);
	}
}
...
void _OptTag(TreeNode *root, Type *_type) {
	...
	{
		_ID((TreeNode *) root->child[0], _type);
	}
}
...
void _FunDec(TreeNode *root, Type *_type) {
	...
	//
	if (root->child_num == 4) {
		_ID((TreeNode *) root->child[0], function_type);
		_VarList((TreeNode *) root->child[2], function_type);
...
void _VarList(TreeNode *root, Type *_type) {
	...
	if (root->child_num == 1) {
		_ParamDec((TreeNode *) root->child[0], &para_type);
		add_function_parameters(_type, &para_type);
	} else {
		_ParamDec((TreeNode *) root->child[0], &para_type);//paramDec
		add_function_parameters(_type, &para_type);
		_VarList((TreeNode *) root->child[2], _type);//Varlist
	}
}
...
void _ParamDec(TreeNode *root, Type *_type) {
	...
	_Specifier((TreeNode *) root->child[0], _type);
	_VarDec((TreeNode *) root->child[1], _type);
}
...
void _CompSt(TreeNode *root) {
	...
	_DefList((TreeNode *) root->child[1]);
	_StmtList((TreeNode *) root->child[2]);
}
...
void _Def(TreeNode *root) {
	...
	initial_type(&_type);
	_Specifier((TreeNode *) root->child[0], &_type);
	_DecList((TreeNode *) root->child[1], &_type);
	_SEMI((TreeNode *) root->child[2]);
}
...
void _DecList(TreeNode *root, Type *_type) {
	...
	if (root->child_num == 1) {
		_Dec((TreeNode *) root->child[0], _type);
	} else //3
	{
		_Dec((TreeNode *) root->child[0], _type);
		_DecList((TreeNode *) root->child[2], _type);
...
void _Dec(TreeNode *root, Type *_type) {
	...
	if (root->child_num == 1) {
		...
		if (is_in_struct == 0) {
			...
			_Exp((TreeNode *) root->child[2], exp_type);
...
void _VarDec(TreeNode *root, Type *_type) {
	...
	if (root->child_num == 1) {
		...
		if (_type->type == Struct_variable) {
			...
			_ID((TreeNode *) root->child[0], variable_name);
...
void _Exp(TreeNode *root, Type *_type) {
	...
	TreeNode *first_child = (TreeNode *) root->child[0];
	if (strcmp(first_child->name, "INT") == 0) {
		_INT((TreeNode *) root->child[0], _type);
		return;
	} else if (strcmp(first_child->name, "FLOAT") == 0) {
		_FLOAT((TreeNode *) root->child[0], _type);
		return;
	} else if (strcmp(first_child->name, "Exp") == 0) {
		TreeNode *second_child = (TreeNode *) root->child[1];
		if (strcmp(second_child->name, "ASSIGNOP") == 0) {
			Type *before = (Type *) malloc(sizeof(Type));
			initial_type(before);
			Type *after = (Type *) malloc(sizeof(Type));
			initial_type(after);
			_Exp((TreeNode *) root->child[0], before);
			_Exp((TreeNode *) root->child[2], after);
		...
		} else if (strcmp(second_child->name, "DOT") == 0)//| Exp DOT ID
		{
			...
			_ID((TreeNode *)root->child[2], after);
		...
		} else if (root->child_num == 4)//ID LP Args RP
		{
			...
			if (check_whether_defined(_type, Functions)) {
				if (check_whether_func(_type)) {
					_Args((TreeNode *)root->child[2], _type);
	...
	{
		_Exp((TreeNode *)root->child[1], _type);
	} else if (strcmp(first_child->name, "MINUS") == 0) {
		_Exp((TreeNode *)root->child[1], _type);
		if (whether_is_variable_or_normal(_type))
			return;
		else
			initial_type(_type);
	} else if (strcmp(first_child->name, "NOT") == 0) {
		_Exp((TreeNode*)root->child[1], _type);
		if (whether_is_variable_or_normal(_type))
			return;
		else
...
void _Args(TreeNode *root, Type *_type) {
	...
	initial_type(&para);
	if (root->child_num == 1) {
		_Exp((TreeNode *)root->child[0], &para);
...
void _StmtList(TreeNode *root) {
	...
	if (root->is_terminal == 0 && root->child_num == 0)
		...
		_Stmt((TreeNode *)root->child[0]);
		_StmtList((TreeNode *)root->child[1]);
	}
}
...
void _Stmt(TreeNode *root) {
	...
	else if (strcmp(first_node->name, "CompSt") == 0)//CompSt
	{
		...
		strcpy(temp_type->id_name, "CompSt");
		Add_symbol_table(temp_type);
		_CompSt((TreeNode *)root->child[0]);
		Delete_current_table();
	} else if (strcmp(first_node->name, "IF") == 0)//IF
	{
		if (root->child_num == 5)//| IF LP Exp RP Stmt
		{
			_Exp((TreeNode *)root->child[2], temp_type);
			_Stmt((TreeNode *)root->child[4]);
		} else//| IF LP Exp RP Stmt ELSE Stmt
		{
			_Exp((TreeNode *)root->child[2], temp_type);
			_Stmt((TreeNode *)root->child[4]);
			_Stmt((TreeNode *)root->child[6]);
		}
